"""Reusable Obsidian MCP server for local Markdown vaults."""
...
def _resolve_write_note_path(path: str, *, must_exist: bool) -> Path:
    raw = str(path or "").strip()
    if not raw:
        raise ValueError("path is required")
    target = _resolve_safe_path(raw, must_exist=False)
    if target.suffix.lower() != ".md":
    ...
    try:
        ...
        raise ValueError("path escapes vault root") from exc
    if _is_hidden(rel):
        raise ValueError("hidden paths are not writable")
    if must_exist and not target.exists():
...
def _create_note_result(
    ...
    target = _resolve_write_note_path(path, must_exist=False)
    ...
    return {
        "action": "created" if not bool(overwrite) else "written",
        "path": target.relative_to(_vault_root()).as_posix(),
        "chars_written": len(content),
        "overwrote_existing": bool(overwrite and existed),
    }


def _update_note_result(path: str, content: str) -> dict[str, Any]:
    if not isinstance(content, str):
        raise ValueError("content must be a string")
    target = _resolve_write_note_path(path, must_exist=True)
    _write_text(target, content)
    return {
        "action": "updated",
        "path": target.relative_to(_vault_root()).as_posix(),
...
def _append_note_result(
    ...
    joiner = "" if not existing or separator is None or existing.endswith(separator) else separator
    updated = f"{existing}{joiner}{content}"
    _write_text(target, updated)
    return {
        "action": "appended",
        "path": target.relative_to(_vault_root()).as_posix(),
        "chars_appended": len(content),
        "content_chars": len(updated),
    }


def _delete_note_result(
    path: str,
    include_deleted_content: bool | None = False,
    max_chars: int | None = None,
) -> dict[str, Any]:
    target = _resolve_write_note_path(path, must_exist=True)
    content = None
    truncated = False
    if bool(include_deleted_content):
...
def _move_note_result(
    source_path: str,
    destination_path: str,
    overwrite: bool | None = False,
    create_dirs: bool | None = True,
) -> dict[str, Any]:
    source = _resolve_write_note_path(source_path, must_exist=True)
    destination = _resolve_write_note_path(destination_path, must_exist=False)
    root = _vault_root()
    ...
    if destination_existed:
        if not bool(overwrite):
            raise ValueError("destination note already exists")
    ...
    return {
        ...
        "overwrote_existing": bool(overwrite and destination_existed),
    }
...
def obsidian_create_note(
    ...
    """Create a Markdown note in the configured vault."""
    return _create_note_result(path, content, overwrite, create_dirs)


@mcp.tool()
def obsidian_update_note(path: str, content: str) -> dict[str, Any]:
    """Replace an existing Markdown note's full content."""
    return _update_note_result(path, content)


@mcp.tool()
def obsidian_append_note(
    path: str,
    content: str,
    separator: str | None = "\n",
) -> dict[str, Any]:
    """Append content to an existing Markdown note."""
    return _append_note_result(path, content, separator)
...
def obsidian_delete_note(
    ...
    """Delete an existing Markdown note from the configured vault."""
    return _delete_note_result(path, include_deleted_content, max_chars)
...
def obsidian_move_note(
    ...
    """Move or rename an existing Markdown note in the configured vault."""
    return _move_note_result(source_path, destination_path, overwrite, create_dirs)
